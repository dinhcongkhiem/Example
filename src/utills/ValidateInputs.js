const validateInputs = (listErr, fields, rules, setListErr) => {
    ...
    rules.forEach(({ field, validations }) => {
        validations.forEach(({ type, matchField }) => {   
            ...
            const value = fields[field]; // Get value from component
            const matchValue = matchField ? fields[matchField] : null; //get MatchValue from fields (from component)
...
export const validateInputSetNewPass = (listErr, fields, setListErr) => validateInputs(listErr, fields, setNewPassRules, setListErr);
